...
void traverseNode(ast_node * current, symbolTable* table, symbolTable* global, int offset){
    if(current->construct== typeDefinition){
        ...
        if(info->union_or_record = TK_RECORD){
            ...
            while(child != NULL){ //child == fieldDefinition
                if(child->firstChild->construct == primitiveDatatype){
                    ...
                    struct ast_node * primitive = child->firstChild;
                    ...
                    if(info2->union_or_record = TK_RECORD){firstChild
                        field->type = RECORD;
